...
public class FileUtil {
    ...
    // 用户头像保存位置
    private final static String HEADPHOTO_PATH = "/Android/data/cn.huiyunche.stan/uploadpic/";
    ...
    public static String getHeadPhotoDir() {
        ...
        String cutime = String.valueOf(ss);
        time = cutime;
     ...
     * 拍照时照片路径
     */
    public static File getPhotoFileTemp() {
        File file = new File(Environment.getExternalStorageDirectory().getAbsolutePath() +
                HEADPHOTO_PATH + time + "/" + HEADPHOTO_NAME_TEMP);
        return file;
    }
    ...
    public  static   File  saveCutBitmapForCache2(Context context, Bitmap bitmap) {
        File file = new File(getHeadPhotoDir() +File.separator + UUID.randomUUID().toString()+".jpg");
        try {
            FileOutputStream out = new FileOutputStream(file);
            bitmap.compress(Bitmap.CompressFormat.JPEG, 85, out);
            out.flush();
